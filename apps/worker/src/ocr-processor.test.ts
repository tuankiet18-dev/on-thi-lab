...
describe("processOcrJob", () => {
  ...
  it("retries only the detected question panel when a full-screen capture misses options", async () => {
    const saveOcrResult = vi.fn(async () => undefined);
    const repository = {
      claimOcrJob: vi.fn(async () => true),
      isOcrJobActive: vi.fn(async () => true),
      ...
      send: vi
        ...
        .mockResolvedValueOnce({
          Blocks: [
            ...
            {
              BlockType: "LINE",
              Text: "Which item is not part of a communication plan?",
              Confidence: 98,
              Geometry: {
                BoundingBox: {
                  Left: 0.4,
          ...
          Blocks: [
            {
              ...
              Confidence: 99,
              Geometry: {
                BoundingBox: { Left: 0.05, Top: 0.1, Width: 0.7, Height: 0.03 },
              },
            },
            {
              BlockType: "LINE",
              Text: "A. Project stakeholders",
              Confidence: 99,
              Geometry: {
                BoundingBox: {
                  Left: 0.05,
    ...
    await processOcrJob(
      {
        questionId,
        revisionId,
        imageKey: "questions/full-screen.png",
        imageHash: "full-screen-hash",
      ...
      },
      {
        ...
        images: {
          read: async () => ({
            bytes: new TextEncoder().encode(
              '<svg width="1600" height="900" xmlns="http://www.w3.org/2000/svg"><rect width="1600" height="900" fill="white"/></svg>',
            ),
            contentType: "image/svg+xml",
          }),
        },
    ...
    expect(textract.send).toHaveBeenCalledTimes(2);
    expect(saveOcrResult).toHaveBeenCalledWith(
      questionId,
      expect.objectContaining({
        options: ["Project stakeholders", "Project budget"],
        status: "approved",
        layout: expect.objectContaining({ retryUsed: true }),
      }),
    );
  });
